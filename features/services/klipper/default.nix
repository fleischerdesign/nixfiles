...
in
{
  ...
  config = lib.mkIf cfg.enable {
      ...
      settings = {
        authorization = {
          ...
          cors_domains = lib.mkMerge [
            ...
            (lib.mkIf (ep.mainsail.proxy.subdomain != null) [
              "https://${ep.mainsail.proxy.subdomain}.${baseDomain}"
            ])
          ];
          trusted_clients = [
            "10.0.0.0/8"
        ...
        "webcam bed" = lib.mkIf (ep.mainsail-cam.proxy.subdomain != null) {
          location = "printer";
          stream_url = "https://${ep.mainsail-cam.proxy.subdomain}.${baseDomain}/?action=stream";
          snapshot_url = "https://${ep.mainsail-cam.proxy.subdomain}.${baseDomain}/?action=snapshot";
          flip_vertical = true;
        };
      };
    };
    ...
    services.caddy.virtualHosts = lib.mkMerge [
      (lib.mkIf (ep.mainsail.proxy.subdomain != null) {
        "${ep.mainsail.proxy.subdomain}.${baseDomain}" = {
          extraConfig = ''
            ...
            route {
              ...
              ${lib.optionalString ep.mainsail.proxy.auth "import authentik"}
      ...
      (lib.mkIf (ep.moonraker.proxy.subdomain != null) {
        "${ep.moonraker.proxy.subdomain}.${baseDomain}" = {
          extraConfig = ''
            reverse_proxy 127.0.0.1:7125
            ${lib.optionalString ep.moonraker.proxy.auth "import authentik"}
          '';
        };
      })
      (lib.mkIf (ep.mainsail-cam.proxy.subdomain != null) {
        "${ep.mainsail-cam.proxy.subdomain}.${baseDomain}" = {
          extraConfig = ''
