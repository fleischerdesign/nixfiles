...
**Status:** Implementation spec for a fresh implementer. **P1 + P2 + the P3/P4 capability-token model + the P5 (tenant-isolation) / P6 (audit) logic primitives are implemented** as the `dsh-capability` plugin (see `plugins/dsh-capability/` and its README): canonicalizer + root automaton + the shared `authorise()` primitive + capability-token model (default-deny), P2 clearance→preset mapping + a fail-closed tool gate, P5 tenant-isolation argv, P6 leak-free audit records. Unit-tested against the matrix; `nix flake check` green. Remaining: the runtime wiring that couples these to the live mesh/process layer (P4 PeerMeshStrategy delegation in dsh-auth, P5 exec-wrapper application, P6 record emission, live service smoke test).
