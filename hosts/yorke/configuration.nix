...
}:
{
  ...
  # Capability authorization is configured on the public gateway (rollins);
  # local client hosts keep the default (enforce off) so the local desktop
  # agent is never locked out.
