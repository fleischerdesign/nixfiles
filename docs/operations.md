...
| `nod switch` activates a generation; **`nod boot` makes it the boot default** | measured 2026-09-22: after five hosts had been deployed to many times, a reboot brought them back on configurations from days earlier (Blocky instead of the resolver, iptables instead of nftables), because only the running system had been switched. Deploy with `nod switch` **and** `nod boot`, or the next reboot silently undoes the fleet |
